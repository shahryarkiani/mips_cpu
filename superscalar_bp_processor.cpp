...
void SuperscalarBpProcessor::advance() {
  ...
  // like the instruction in A's decode should be older than B's decode, and so
  // on for the other stages

  // Writeback
  {
    int write_reg_a = wb_in_a.link       ? 31
                      : wb_in_a.reg_dest ? wb_in_a.rd
                                         : wb_in_a.rt;
    uint32_t write_data_a = wb_in_a.link         ? regfile.pc + 8
                            : wb_in_a.mem_to_reg ? wb_in_a.read_data_mem
                                                 : wb_in_a.alu_result;
    int write_reg_b = wb_in_b.link       ? 31
                      : wb_in_b.reg_dest ? wb_in_b.rd
                                         : wb_in_b.rt;
    uint32_t write_data_b = wb_in_b.link         ? regfile.pc + 8
                            : wb_in_b.mem_to_reg ? wb_in_b.read_data_mem
                                                 : wb_in_b.alu_result;

    DEBUG(cout << "wb_a fetch_pc = " << wb_in_a.pc << ", r" << write_reg_a
               << " = " << write_data_a << "\n";)
    DEBUG(cout << "wb_b fetch_pc = " << wb_in_b.pc << ", r" << write_reg_b
               << " = " << write_data_b << "\n";)

    // Write Back
    regfile.access(0, 0, wb_in_a.read_data_2, wb_in_a.read_data_2, write_reg_a,
                   wb_in_a.reg_write, write_data_a);
    regfile.access(0, 0, wb_in_b.read_data_2, wb_in_b.read_data_2, write_reg_b,
                   wb_in_b.reg_write, write_data_b);
  ...
  // Instruction Decode
  {
    ...
    regfile.access(id_out_a.rs, id_out_a.rt, id_out_a.read_data_1,
                   id_out_a.read_data_2, 0, 0, 0);
    regfile.access(id_out_b.rs, id_out_b.rt, id_out_b.read_data_1,
                   id_out_b.read_data_2, 0, 0, 0);

    DEBUG(cout << "id_out_a: fetch_pc = " << id_out_a.pc << ", r" << id_out_a.rs
               << " = " << id_out_a.read_data_1 << ", r" << id_out_a.rt << " = "
               << id_out_a.read_data_2 << "\n";)
    DEBUG(cout << "id_out_b: fetch_pc = " << id_out_b.pc << ", r" << id_out_b.rs
               << " = " << id_out_b.read_data_1 << ", r" << id_out_b.rt << " = "
               << id_out_b.read_data_2 << "\n";)
    id_out_a.imm = id_out_a.zero_extend   ? id_out_a.imm
                   : (id_out_a.imm >> 15) ? 0xffff0000 | id_out_a.imm
                                          : id_out_a.imm;
    id_out_b.imm = id_out_b.zero_extend   ? id_out_b.imm
                   : (id_out_b.imm >> 15) ? 0xffff0000 | id_out_b.imm
                                          : id_out_b.imm;
  ...
  {
    if ((ex_in_a.mem_read &&
         (ex_in_a.rt == id_out_a.rs || (ex_in_a.rt == id_out_a.rt))) ||
        (ex_in_b.mem_read &&
         (ex_in_b.rt == id_out_a.rs || (ex_in_b.rt == id_out_a.rt)))) {
      load_use_stall_a = true;
    }  // Are we using a register that one of the previous instruction pairs is
       // loading into?
    if ((ex_in_a.mem_read &&
         (ex_in_a.rt == id_out_b.rs || (ex_in_a.rt == id_out_b.rt))) ||
        (ex_in_b.mem_read &&
         (ex_in_b.rt == id_out_b.rs || (ex_in_b.rt == id_out_b.rt)))) {
      load_use_stall_b = true;
    }
    load_use_stall = load_use_stall_a || load_use_stall_b;

    // Maybe need to rethink ordering of this, we should first try to forward
    // from B wb, then B mem, then from A wb then from A mem Forwarding for
    // pipeline A, first we check if we need to forward from the previous A's wb
    int reg_write = wb_in_a.reg_dest ? wb_in_a.rd : wb_in_a.rt;
    if (wb_in_a.reg_write) {  // Forward to pipelineA from wb A
      auto read_data =
          wb_in_a.mem_to_reg ? wb_in_a.read_data_mem : wb_in_a.alu_result;
      if (reg_write == ex_in_a.rs) {
        ex_in_a.read_data_1 = read_data;
      }
      if (reg_write == ex_in_a.rt) {
        ex_in_a.read_data_2 = read_data;
      }
    }
    ...
    if (wb_in_b.reg_write) {  // Forward to pipeline A from wb B
      auto read_data =
          wb_in_b.mem_to_reg ? wb_in_b.read_data_mem : wb_in_b.alu_result;
      if (reg_write == ex_in_a.rs) {
        ex_in_a.read_data_1 = read_data;
      }
      if (reg_write == ex_in_a.rt) {
    ...
    if (wb_in_a.reg_write) {  // Forward to pipeline B from wb A
      auto read_data =
          wb_in_a.mem_to_reg ? wb_in_a.read_data_mem : wb_in_a.alu_result;
      if (reg_write == ex_in_b.rs) {
        ex_in_b.read_data_1 = read_data;
      }
      if (reg_write == ex_in_b.rt) {
        ex_in_b.read_data_2 = read_data;
      }
    }
    reg_write = wb_in_b.reg_dest ? wb_in_b.rd : wb_in_b.rt;
    ...
    if (mem_in_a.reg_write) {  // Forward to pipeline A from mem A
      if (reg_write == ex_in_a.rs) {
        ex_in_a.read_data_1 = mem_in_a.alu_result;
      }
      if (reg_write == ex_in_a.rt) {
        ex_in_a.read_data_2 = mem_in_a.alu_result;
      }
    }
    reg_write = mem_in_b.reg_dest ? mem_in_b.rd : mem_in_b.rt;
    if (mem_in_b.reg_write) {  // Forward to pipeline A from mem B
      if (reg_write == ex_in_a.rs) {
        ex_in_a.read_data_1 = mem_in_b.alu_result;
      }
      if (reg_write == ex_in_a.rt) {
        ex_in_a.read_data_2 = mem_in_b.alu_result;
      }
    }
    ...
    if (mem_in_a.reg_write) {  // Forward to pipeline B from mem A
      if (reg_write == ex_in_b.rs) {
        ex_in_b.read_data_1 = mem_in_a.alu_result;
      }
      if (reg_write == ex_in_b.rt) {
        ex_in_b.read_data_2 = mem_in_a.alu_result;
      }
    }
    reg_write = mem_in_b.reg_dest ? mem_in_b.rd : mem_in_b.rt;
    if (mem_in_b.reg_write) {  // Forward to pipeline B from mem B
      if (reg_write == ex_in_b.rs) {
        ex_in_b.read_data_1 = mem_in_b.alu_result;
      }
      if (reg_write == ex_in_b.rt) {
        ex_in_b.read_data_2 = mem_in_b.alu_result;
      }
    }

    uint32_t alu_zero = 0;

    alu_a.generate_control_inputs(ex_in_a.ALU_op, ex_in_a.funct,
                                  ex_in_a.opcode);
    ...
    alu_b.generate_control_inputs(ex_in_b.ALU_op, ex_in_b.funct,
                                  ex_in_b.opcode);
    ...
    DEBUG(cout << "alu_a: fetch_pc = " << ex_in_a.pc
               << ", opA = " << operand_1_a << ", opB = " << operand_2_a
               << ", result = " << alu_result_a << "\n";)
    DEBUG(cout << "alu_b: fetch_pc = " << ex_in_b.pc
               << ", opA = " << operand_1_b << ", opB = " << operand_2_b
               << ", result = " << alu_result_b << "\n";)

    // // This is to work with the original code
    // if(ex_in.branch && ((ex_in.bne && !alu_zero) || (!ex_in.bne &&
    // alu_zero))) {
    //     dont_fetch = true;
    // }
  ...
  // Memory
  {
    // If detect branch misprediction in pipeline A, we've got to flush out B's
    // mem, and also flush out the F D E parts of both pipeline A and B If it's
    // detected in pipeline B, we just need to flush out F D E for both For both
    // cases, update the PC accordingly Special case: If a branch misprediction
    // happens in both pipeline A + B, we want the PC from A, not B Special
    // case: Dependent stalls interact weirdly with branch misprediction, if
    // there's been a misprediction We don't want to dependent stall, because
    // those instructions are being thrown out anyways
    if (mem_in_b.branch) {
      bool actual_taken = ((mem_in_b.bne && mem_in_b.alu_result) ||
                           (!mem_in_b.bne && !mem_in_b.alu_result));
      ...
      predictor.recordBranch(mem_in_b.pc, target, actual_taken, mem_in_b.taken,
                             mem_in_b.predict_sum);
    }

    if (mem_in_a.branch) {
      bool actual_taken = ((mem_in_a.bne && mem_in_a.alu_result) ||
                           (!mem_in_a.bne && !mem_in_a.alu_result));
      ...
      predictor.recordBranch(mem_in_a.pc, target, actual_taken, mem_in_a.taken,
                             mem_in_a.predict_sum);
    }

    // If we have mem stall, we need to stall both pipelines, since we don't
    // have reorder buffer
    ...
    bool mem_success_a =
        memory->access(mem_in_a.alu_result, read_data_mem_a, 0,
                       mem_in_a.mem_read | mem_in_a.mem_write, 0);

    DEBUG(cout << "mem_a fetch_pc = " << mem_in_a.pc
               << ", addr = " << mem_in_a.alu_result << "\n";)
    DEBUG(cout << "mem_b fetch_pc = " << mem_in_b.pc
               << ", addr = " << mem_in_b.alu_result << "\n";)

    if (mem_success_a) {
      // Stores: sb or sh mask and preserve original leftmost bits
      write_data_mem_a = mem_in_a.halfword ? (read_data_mem_a & 0xffff0000) |
                                                 (mem_in_a.read_data_2 & 0xffff)
                         : mem_in_a.byte ? (read_data_mem_a & 0xffffff00) |
                                               (mem_in_a.read_data_2 & 0xff)
                                         : mem_in_a.read_data_2;
      ...
      read_data_mem_a &= mem_in_a.halfword ? 0xffff
                         : mem_in_a.byte   ? 0xff
                                           : 0xffffffff;

      bool mem_success_b =
          memory->access(mem_in_b.alu_result, read_data_mem_b, 0,
      ...
      if (mem_success_b) {
        write_data_mem_b =
            mem_in_b.halfword ? (read_data_mem_b & 0xffff0000) |
                                    (mem_in_b.read_data_2 & 0xffff)
            : mem_in_b.byte
                ? (read_data_mem_b & 0xffffff00) | (mem_in_b.read_data_2 & 0xff)
                : mem_in_b.read_data_2;
        memory->access(mem_in_b.alu_result, read_data_mem_b, write_data_mem_b,
                       mem_in_b.mem_read, mem_in_b.mem_write);

        read_data_mem_b &= mem_in_b.halfword ? 0xffff
                           : mem_in_b.byte   ? 0xff
                                             : 0xffffffff;
        ...
        for (auto& fetch_addr : recent_fetchs) {
          if (mem_in_a.mem_write && a_addr == fetch_addr) {
            ...
            if (mem_in_b.pc != b_addr) mem_fetch_conflict_b = true;
          }
        }
  ...
  // Instruction Fetch
  {
    if (!mem_stall && !dont_fetch && !load_use_stall &&
        !(mem_fetch_conflict_a || mem_fetch_conflict_b)) {
      bool mem_success_a =
          memory->access(fetch_pc, if_out_a.instruction, 0, 1, 0);
      if (!mem_success_a) {
        if_stall = true;
      } else {
        bool mem_success_b =
            memory->access(fetch_pc + 4, if_out_b.instruction, 0, 1, 0);
        if (!mem_success_b) {
          if_stall = true;
        }
